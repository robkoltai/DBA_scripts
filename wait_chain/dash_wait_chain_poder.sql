...
--@c "'Username:'||username||'  Program:'||program2||'  SQL_ID:'||sql_id||'  object,file,block:'||CURRENT_OBJ#||','||CURRENT_file#||','||CURRENT_block#||'  Event:'||event2" "lower(event)  like '%tx - row lock%'"  "timestamp'2015-08-10 20:00:01'" "timestamp'2015-08-10 23:59:59'" 
--@c username||':'||program2||event2 "lower(program)  like '%%'"  "timestamp'2015-08-10 23:30:01'" "timestamp'2015-08-10 23:50:59'"
...
SELECT
    LPAD(ROUND(RATIO_TO_REPORT(COUNT(*)) OVER () * 100,1)||'%',5,' ') "%This"
  , COUNT(*) * 10 seconds
  , ROUND(COUNT(*) * 10 / ((CAST(&4 AS DATE) - CAST(&3 AS DATE)) * 86400), 2) AAS
  , path wait_chain
  -- , COUNT(DISTINCT sids)
  -- , MIN(sids)
  -- , MAX(sids)
